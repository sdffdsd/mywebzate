...
| 推送即部署 | ✅ 已实现：`git push` → GitHub Actions 构建 → `wrangler pages deploy` 上线（`.github/workflows/deploy.yml`）。<br>本项目是 **Direct Upload 项目**，Cloudflare 不允许转成 Git 集成（API 报 `You cannot update the source object in a Direct Uploads project`），所以走 Actions 路线。<br>所需 secrets 已配置：`CLOUDFLARE_API_TOKEN`（Account → Cloudflare Pages → Edit）与 `CLOUDFLARE_ACCOUNT_ID`；未配置时部署步骤自动跳过、CI 仍为绿。 |
